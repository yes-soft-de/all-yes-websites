<?php
	...
	*/
	...
	function yes_idea_project_custom_post_type() {
		// All Post Labels
		$label = array(
			'name'                => 'Projects',
...
//		'taxonomies'          => array( 'category', 'post_tag' ),
		...
		register_post_type( 'yes_idea_project', $args );
	}
	add_action( 'init', 'yes_idea_project_custom_post_type' );


	/*
	 * Function To Add New Taxonomy Hierarchical And NOT Hierarchical
	 */
	function yes_idea_project_custom_taxonomies() {
		...
		$args = array(
			...
			'query_var' => true,
			'rewrite' => array( 'slug' => 'yes_idea_project_category' )
		);

		register_taxonomy('yes_idea_project_category', 'yes_idea_project', $args);

		//add new taxonomy NOT hierarchical

		register_taxonomy('project_tag', 'yes_idea_project', array(
			'label' => 'Project Tag',
			'rewrite' => array( 'slug' => 'project_tag' ),
			'hierarchical' => false
		) );
	}
	add_action( 'init' , 'yes_idea_project_custom_taxonomies' );


	/* Contact Custom Port Type */
	function yes_idea_contact_custom_post_type() {
		$labels = array(
			'name'              => pll_('Messages'),
		...
		register_post_type( 'yes-idea-contact', $args );
	}
	// hook for display our contact custom post in admin menu
	add_action( 'init', 'yes_idea_contact_custom_post_type' );

	/*
	* function to create new columns and rename all columns as we want
	*/
	function yes_idea_set_contact_columns( $columns ) {
		$newColumns = array();
	...
	add_filter( 'manage_yes-idea-contact_posts_columns', 'yes_idea_set_contact_columns' );

	/*
	 * Function to print the value for every columns
	 */
	function yes_idea_contact_custom_column( $column, $post_id ) {
		switch ( $column ) {
			case 'message' :
	...
	add_action( 'manage_yes-idea-contact_posts_custom_column', 'yes_idea_contact_custom_column', 10, 2 );
	...
	function yes_idea_contact_add_meta_box() {
		add_meta_box( 'contact_email', pll_('User Email'), 'yes_idea_contact_email_callback', 'yes-idea-contact', 'side', 'default' );
	}
	// action to activate my function and generate the meta box
	add_action( 'add_meta_boxes', 'yes_idea_contact_add_meta_box' );

	// $post: will automatically pass by add_meta_box and it will care all information related to 'yes_idea-contact' post type
	function yes_idea_contact_email_callback( $post ) {
		wp_nonce_field( 'yes_idea_save_contact_email_data', 'yes_idea_contact_email_meta_box_nonce' );
		$value = get_post_meta( $post->ID, '_contact_email_value_key', true );
		// Print The input as wordpress theme build-in
		echo '<label for="yes_idea_contact_email_field">' . pll_('User Email Address') . ': </label>';
		echo '<input type="email" id="yes_idea_contact_email_field" name="yes_idea_contact_email_field" value="' . esc_attr( $value ) . '" size="25" />';
	}

// function to print the email inside our meta box and save the post
	function yes_idea_save_contact_email_data( $post_id ) {
		// check if the nonce is not set in the $_POST Method
		if ( ! isset( $_POST['yes_idea_contact_email_meta_box_nonce'] ) ) {
			return;
		}
		// check if the nonce is valid and nonce is generated by wordpress and not generated by and hacker or another user
		if ( ! wp_verify_nonce( $_POST['yes_idea_contact_email_meta_box_nonce'], 'yes_idea_save_contact_email_data' ) ) {
			return;
		}
		...
		// check if our input name yes_idea_contact_email_field is in $_POST method
		if ( ! isset( $_POST['yes_idea_contact_email_field'] ) ) {
			return;
		}
		// Retrieve the data
		$my_data = sanitize_text_field( $_POST['yes_idea_contact_email_field'] );
		update_post_meta( $post_id, '_contact_email_value_key', $my_data );
	}
	// action to save the post in our contact custom post
	add_action( 'save_post', 'yes_idea_save_contact_email_data' );




	// Posts Relation with products
	function yes_idea_product_add_meta_boxes() {
		add_meta_box( 'product_meta_box', pll_( 'Products Relationship' ), 'yes_idea_products_callback', 'post', 'side', 'default' );
	}
	add_action( 'add_meta_boxes', 'yes_idea_product_add_meta_boxes' );


	function yes_idea_products_callback( $post ) {
	...
	add_action( 'save_post', 'yes_idea_save_product_field' );
	function yes_idea_save_product_field( $post_id ) {
